...
/**
 * @author Chetna
 *reading files of details
 */
public class Processing {

	BufferedReader br;

	final static String DETAILS_FILE = "./src/SocialNetworkingSystem/PersonDetails.txt";
	final static String FRIEND_FILE = "./src/SocialNetworkingSystem/FriendList.txt";
		...
		try {
			...
			try {
				while ((line = br.readLine()) != null) {
					...
					Iterator<Entity> itr1 = nodeSet.iterator(); // Iterating
																// list of
																// questions
					while (itr1.hasNext()) {
						if (itr1.next().getEmail().equals(email[0])) {
							for (int index = 1; index < email.length; index++) {
								Iterator<Entity> itr = nodeSet.iterator(); // Iterating
																			// list
																			// of
																			// questions
								while (itr.hasNext()) {
									if (itr.next().getEmail()
											.equals(email[index])) {
