...
public class VehicleDBHelper {
	...
	protected void insert(Connection connection, Vehicle vehicle)
		...
		try {
			...
			preparedStatement.setBigDecimal(8,
					BigDecimal.valueOf(vehicle.getRoadTax()));
			preparedStatement.setBigDecimal(9,
					BigDecimal.valueOf(vehicle.getOnRoadPrice()));
			preparedStatement.setString(10, vehicle.getCreatedBy());
			preparedStatement.setTimestamp(11, new Timestamp(vehicle
	...
	public List<Vehicle> getVehiclesByBrand(Connection connection, String brand)
		...
		if (connection != null) {
			...
			try {
				preparedStatement = connection
						.prepareStatement(SELECT_FOR_BRAND_QUERY);
	...
	public List<Vehicle> getAllVehicle(Connection connection)
		...
		if (connection != null) {
			...
			try {
				preparedStatement = connection
						.prepareStatement(SELECT_FOR_ALL_QUERY);
				resultSet = preparedStatement.executeQuery();
				vehicleList = ModelHelper.createVehicleList(resultSet);
			} catch (SQLException e) {
				throw new CarDekhoSystemException(
	...
	public List<Vehicle> getVehicleBrands(Connection connection)
		...
		if (connection != null) {
			...
			try {
				preparedStatement = connection
						.prepareStatement(SELECT_FOR_ALL_MAKE_QUERY);
				resultSet = preparedStatement.executeQuery();
